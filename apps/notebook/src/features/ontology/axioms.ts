...
export const evaluateOntologyAxioms = (
  ...
  // -------------------------------------------------------------
  // 2. Mark affected edges with isAxiomViolation
  // -------------------------------------------------------------
  const violatingEdgeIdSet = new Set<string>();
  for (const v of violations) {
    if (v.affectedEdgeIds) {
      for (const eid of v.affectedEdgeIds) {
        violatingEdgeIdSet.add(eid);
      }
    }
  }

  for (const edge of edges) {
    if (violatingEdgeIdSet.has(edge.id)) {
      edge.isAxiomViolation = true;
    }
  }

  // -------------------------------------------------------------
  // 3. LOGICAL ONTOLOGY INFERENCE
  // -------------------------------------------------------------
  ...
  const addInferredEdge = (
    ...
    const edge: InferredOntologyEdge = {
      ...
      label: INFERRED_RELATIONS[type].label,
      dashed: true,
      color: '#9B59B6',
    };
  ...
  // Helper to find all ancestor classes for a given class ID
