...
const output = mkdtempSync(path.join(tmpdir(), 'ontology-rdf-'));
...
test('serializes ontology entities, standard predicates and typed data values', () => {
  ...
  assert.match(turtle, /:node:class%3Aboard> rdfs:subClassOf bt:Card/);
  assert.match(turtle, /:node:class%3Alinked-paragraph> rdfs:subClassOf bt:ConnectedParagraph/);
  assert.doesNotMatch(
    turtle,
    /:node:class%3Alinked-paragraph> owl:equivalentClass bt:ConnectedParagraph/
  );
