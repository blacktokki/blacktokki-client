...
const output = mkdtempSync(path.join(tmpdir(), 'ontology-canvas-test-'));
...
try {
  for (const name of ['relations', 'titleKeywordClasses', 'axioms', 'forceLayout', 'canvasRenderer']) {
  ...
  ({ initForceSimulation, stepForceSimulation, computeForceLayout } = require(path.join(output, 'forceLayout.js')));
  ({ screenToWorld, worldToScreen, findNodeAtScreenCoord, drawEdges } = require(path.join(output, 'canvasRenderer.js')));
} finally {
...
test('screenToWorld and worldToScreen maintain mathematical invertibility', () => {
  ...
  for (const wp of testWorldPoints) {
    ...
    assert.ok(Math.abs(restored.x - wp.x) < 1e-5, `World X inverted accurately: ${restored.x} vs ${wp.x}`);
    assert.ok(Math.abs(restored.y - wp.y) < 1e-5, `World Y inverted accurately: ${restored.y} vs ${wp.y}`);
  }
});
...
test('findNodeAtScreenCoord precisely hits circular and rectangular literal nodes', () => {
  ...
  assert.equal(findNodeAtScreenCoord(nodes, 500 + 30 + 3, 500 + 12 + 2, panX, panY, zoom)?.id, 'node:literal');
...
test('findNodeAtScreenCoord performs sub-millisecond hit-testing on 500 nodes', () => {
  ...
  assert.ok(perCheckMs < 0.1, `Hit testing is sub-millisecond: ${perCheckMs.toFixed(4)}ms per check`);
});
...
test('stepForceSimulation incrementally settles 500 nodes and 1000 edges smoothly', () => {
  ...
  assert.ok(
    ...
    `Simulation settled smoothly: early=${earlyMovement.toFixed(2)}, late=${lateMovement.toFixed(2)}`
  );
...
test('evenly distributes topic classes across the canvas and clusters paragraph instances around their parent topic', () => {
  ...
  for (let i = 0; i < topicCount; i++) {
    for (let j = i + 1; j < topicCount; j++) {
      totalInterTopicDist += Math.hypot(topicNodes[i].x - topicNodes[j].x, topicNodes[i].y - topicNodes[j].y);
      pairCount++;
    }
  }
  const avgInterTopicDist = totalInterTopicDist / pairCount;
  assert.ok(avgInterTopicDist > 200, `Topic classes are evenly spread across field (avg dist=${avgInterTopicDist.toFixed(1)}px > 200px)`);
  ...
  for (let t = 0; t < topicCount; t++) {
    ...
    for (let p = 0; p < parasPerTopic; p++) {
      ...
      const otherTopicNode = context.simNodes.find((n) => n.id === `class:topic:${(t + 1) % topicCount}`);
  ...
  assert.ok(
    ...
    `Paragraphs are clustered around their parent topic: own=${avgOwnDist.toFixed(1)}px, other=${avgOtherDist.toFixed(1)}px`
  );
});
