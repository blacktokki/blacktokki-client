...
try {
  ...
  ({ buildTopicClusterIndex } = layoutModule);
...
test('status classes, note annotations and references do not create inferences', () => {
  const nodes = [
  ...
  const result = evaluateOntologyAxioms(nodes, [
    ...
    edge('status-class', 'done-note', 'REPRESENTED_BY_NOTE'),
...
test('containment remains asserted and is not exposed as an inferred predicate', () => {
  assert.equal(getRelationType('PART_OF'), 'PART_OF');
  assert.equal(isInferredRelationType('INFERRED_PART_OF'), false);
});
...
test('all inferred predicates are recognizable and have finite force-layout coordinates', () => {
  ...
  for (const relation of ['INSTANCE_OF', 'SUBCLASS_OF']) {
