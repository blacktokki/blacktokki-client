...
3. 각 기능 간의 종속 관계는  (`types` or `styles`) ◀ (`component` or (`services` ◀ `hook`)) ◀ `modals` ◀ (`screens` or `features`) ◀ `navigation` 순서로 적용한다.

## 프레임워크 및 라이브러리
### expo / react-native / react-native-web
1. PC 웹을 기준으로 우선 개발하되, 가능한 모바일 웹, 모바일 앱에 호환되는 코드를 작성해야 한다.
### react-navigation
1. naviagtion 사용시 일반적인 화면 전환이나 메인 탭 이동 시에는 기존 스택 화면을 재사용하고 중복을 방지하는 `navigate()`를 사용한다.
2. naviagtion 사용시 상세 페이지 내에서 또 다른 상세 페이지로 이동하는 등 동일한 구조의 화면을 중첩해서 쌓아야 할 때는 `push()`를 사용한다.
3. naviagtion 사용시 사용자가 '뒤로 가기'를 통해 이전 탐색 이력을 순차적으로 확인해야 하는 '순환적 탐색' 구조에서는 반드시 `push()`를 적용해야 한다.
4. naviagtion 사용시 불필요한 메모리 낭비와 스택 오버플로우를 막기 위해, 화면의 고유성이 보장되어야 하는 경우에는 `navigate()` 사용을 권장한다.
### react-query
...
5. 모달을 닫을 때는 `setModal(null)`을 사용한다.
### @blacktokki/account
...
4. `Content.description`은 HTML 문자열로 저장된 노트의 본문을 포함한다.
...
1. 이 서비스의 부가가능은 `/features`에 각 부가기능 별로 포함한다.
...
3. `paragraphDescription(...)`을 사용해 문단(하위 문단 포함 또는 제외)에 해당하는 HTML만 추출한다.
4. 추가 및 변경된 기능은 사용자 가이드(사용 방법.md)에 마크다운 포맷으로 반영하여야 한다.
5. 기존 코드를 수정할 시 불필요한 변경 사항을 최소화 한다.(공백, 줄바꿈, 주석, console.log 등)
