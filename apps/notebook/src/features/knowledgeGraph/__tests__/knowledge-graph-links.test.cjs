...
const output = mkdtempSync(path.join(tmpdir(), 'knowledge-graph-links-'));
let findOntologyLinkSourceNodeId;
let findOntologyLinkTargetNodeId;
try {
  const source = readFileSync(path.join(__dirname, '../utils/links.ts'), 'utf8');
...
test('resolves a board paragraph before its board note', () => {
  const boardNodes = [
    { ...nodes[0], id: 'board-note', name: '프로젝트/진행', noteTitle: '프로젝트/진행' },
    {
      ...nodes[1],
      id: 'board-paragraph',
      name: '작업 묶음',
  ...
  assert.equal(
    findOntologyLinkTargetNodeId(boardNodes, new Map([['프로젝트/진행', 'board-note']]), {
      title: '프로젝트/진행',
      paragraph: '작업 묶음',
    }),
    'board-paragraph'
...
test('finds every source and destination of a shared board paragraph', () => {
  const shared = {
    id: 'board-paragraph',
    name: '작업 묶음',
    role: 'INSTANCE',
    ...
    paragraphOccurrences: [
      { ...paragraph, origin: '프로젝트/진행', path: 'first', autoSection: '첫 번째' },
      { ...paragraph, origin: '프로젝트/완료', path: 'second', autoSection: '두 번째' },
    ],
  };
  const second = { ...paragraph, path: 'second', autoSection: '두 번째' };
  assert.equal(findOntologyLinkSourceNodeId([shared], '프로젝트/완료', second), shared.id);
  assert.equal(
    findOntologyLinkTargetNodeId([shared], new Map(), {
      title: '프로젝트/완료',
      paragraph: '작업 묶음',
      section: '두 번째',
    }),
